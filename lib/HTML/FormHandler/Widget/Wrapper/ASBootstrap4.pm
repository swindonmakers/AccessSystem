package HTML::FormHandler::Widget::Wrapper::ASBootstrap4;
...
sub wrap_field {
    ...
    # render the label
    $output .= "\n" . $self->do_render_label($result, undef, ['col-form-label'] )
        if $self->do_label;
    ...
    # add any post-element content
    $output .= $self->get_tag('after_element_wrapper');
    # close wrapper
    if ( $self->do_wrapper ) {
        $output .= "\n</div>";
...
sub add_standard_element_classes {
    ...
    push @$class, 'is-invalid' if $result->has_errors;
    push @$class, 'is-invalid' if $result->has_warnings; # doesnt exist in 4?
    ...
    # TODO: is-valid for successful elements?
}
...
sub wrap_checkbox {
    ...
    return qq{<label class="form-check-label" $for>$rendered_widget\n$label\n</label>}
